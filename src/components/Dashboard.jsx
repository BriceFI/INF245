...
export default function Dashboard({ session }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (myCompany && !myCompany.is_admin) {
  ...
  useEffect(() => {
    ...
    // Listen for instant admin broadcast (primary — no replication needed)
    broadcastRef.current = supabase.channel('question_broadcast')
      .on('broadcast', { event: 'question_update' }, () => fetchData())
      .subscribe();
  ...
  useEffect(() => {
    if (activeQuestion && !hasAnswered && timeLeft > 0) {
  ...
  }, [activeQuestion, hasAnswered, timeLeft]);
    ...
    try {
      if (activeQuestion.is_poll) {
        ...
        if (error) {
          ...
          broadcastRef.current?.send({ type: 'broadcast', event: 'question_update', payload: {} });
          if (isCorrect) {
            setResultModal({ type: 'success', message: 'Analyse Correcte', amount: `+ ${formatCurrency(activeQuestion.reward)}` });
          } else {
            setResultModal({ type: 'error', message: 'Erreur Stratégique', amount: `- ${formatCurrency(activeQuestion.penalty)}` });
          }
        }
      }
    } finally {
  ...
  return (
    <div className="h-screen w-full bg-beige-50 flex flex-col relative overflow-hidden text-neutral-800">
      ...
      <div className="absolute top-3 md:top-6 left-3 md:left-6 right-3 md:right-6 z-30 flex items-center justify-between pointer-events-none">
        <div className="flex items-center gap-3 bg-white/80 backdrop-blur-xl p-2 md:p-3 pl-3 md:pl-4 pr-4 md:pr-6 rounded-2xl border border-white/50 shadow-2xl premium-shadow pointer-events-auto">
          ...
          <div className="hidden sm:block">
            <h1 className="text-[10px] font-black uppercase tracking-[0.2em] text-terracotta-600/70 leading-none mb-1">Supervision Projets</h1>
            <div className="text-lg font-black tracking-tight text-neutral-900 leading-none">{company?.company_name || 'Chargement...'}</div>
          </div>
          <div className="block sm:hidden">
            <div className="text-sm font-black tracking-tight text-neutral-900 leading-none">{company?.company_name || '...'}</div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 md:gap-4 pointer-events-auto">
          <div className="bg-white/80 backdrop-blur-xl px-3 md:px-6 py-2 md:py-3 rounded-2xl border border-white/50 shadow-2xl premium-shadow flex flex-col items-end">
            <span className="text-[8px] md:text-[10px] font-black uppercase tracking-widest text-neutral-400 mb-0.5">Capital</span>
            <span className={`text-lg md:text-2xl font-black tracking-tighter tabular-nums ${company?.balance < 0 ? 'text-red-500' : 'text-terracotta-700'}`}>
              {formatCurrency(company?.balance || 0)}
            </span>
          </div>
          <button onClick={() => setLeaderboardOpen(true)} className="w-10 h-10 md:w-14 md:h-14 bg-white/80 backdrop-blur-xl border border-white/50 hover:border-terracotta-200 text-neutral-600 rounded-2xl flex items-center justify-center transition-all shadow-2xl active:scale-95"><Trophy size={20} /></button>
      ...
      <main className="flex-1 mt-20 md:mt-28 mb-4 md:mb-6 mx-3 md:mx-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 relative">
        <RoomCard title="Unité d'Assemblage" delay="0s"> <RoboticAssembly /> </RoomCard>
        <RoomCard title="Flux Logistique" delay="0.1s"> <LogisticsBay /> </RoomCard>
        <RoomCard title="Laboratoire R&D" delay="0.2s"> <ResearchLab /> </RoomCard>
        <RoomCard title="Centre Hydroponique" delay="0.3s"> <HydroponicsBay /> </RoomCard>
        <RoomCard title="Génération Énergie" delay="0.4s"> <PowerStation /> </RoomCard>
        <RoomCard title="Système de Stockage" delay="0.5s"> <AutomatedStorage /> </RoomCard>
      </main>
      ...
      {/* LEADERBOARD */}
      {isLeaderboardOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-beige-900/20 backdrop-blur-sm entrance-anim">
          <div className="bg-white rounded-[2.5rem] w-full max-w-xl shadow-2xl border border-beige-100 overflow-hidden flex flex-col max-h-[80vh]">
            <div className="p-8 pb-4 flex justify-between items-center">
              <div>
                <h3 className="text-xs font-black uppercase tracking-[0.2em] text-terracotta-600 mb-1">Classement Global</h3>
                <h2 className="text-2xl font-black text-neutral-900 tracking-tight">Performances Entités</h2>
              </div>
              <button onClick={() => setLeaderboardOpen(false)} className="p-2 hover:bg-beige-50 rounded-xl text-neutral-400 hover:text-neutral-900 transition-all"><X size={28} /></button>
            </div>
            <div className="px-8 pb-8 overflow-y-auto space-y-2">
              {companies.filter(c => !c.is_admin).map((c, idx) => (
                <div key={c.id} className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${c.id === company?.id ? 'bg-terracotta-50 border-terracotta-200' : 'bg-white border-transparent hover:border-beige-200 hover:bg-beige-50'}`}>
                  <div className="flex items-center gap-4">
                    <span className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-black ${idx === 0 ? 'bg-amber-400 text-white' : idx === 1 ? 'bg-slate-300 text-white' : idx === 2 ? 'bg-orange-400 text-white' : 'text-neutral-400'}`}>{idx + 1}</span>
                    <div className="font-bold text-neutral-900">{c.company_name}</div>
                  </div>
                  <div className={`font-black tabular-nums ${c.balance < 0 ? 'text-red-500' : 'text-neutral-900'}`}>{formatCurrency(c.balance)}</div>
                </div>
              ))}
            </div>
          </div>
