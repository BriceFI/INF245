...
export default function AdminView({ companies }) {
  ...
  const handleOpenDetail = async (q, e) => {
    ...
    const enriched = (aData || []).map(a => {
      const company = companies.find(c => c.user_id === a.user_id);
      return {
        ...a,
        company_name: company?.company_name || 'Inconnu',
  ...
  useEffect(() => {
    ...
    broadcastChannel.current = supabase.channel('question_broadcast')
      .on('broadcast', { event: 'question_update' }, () => fetchActiveQuestion())
      .subscribe();

    const channel = supabase.channel('admin_realtime_full')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'questions' }, () => fetchActiveQuestion())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'answers' }, () => fetchActiveQuestion())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'companies' }, () => fetchActiveQuestion())
      .subscribe();
  ...
  }, [activeQuestion, timeLeft, handleClose]);
      ...
      <main className="flex-1 flex flex-col h-full overflow-y-auto pt-20 md:pt-28 px-4 md:px-8 pb-24 md:pb-12">
        ...
        {currentTab === 'stats' && (
          <div className="max-w-5xl mx-auto w-full space-y-8">
            ...
            <div className="bg-white rounded-[2.5rem] shadow-xl border border-beige-200 overflow-hidden">
              ...
              <table className="w-full text-left border-collapse min-w-[500px]">
                <thead className="bg-beige-50 border-b border-beige-100">
                  <tr>
                    ...
                    <th className="px-8 py-6 text-[10px] font-black uppercase text-neutral-400">Compagnie</th>
                    <th className="px-8 py-6 text-[10px] font-black uppercase text-neutral-400">Responsable</th>
                    <th className="px-8 py-6 text-[10px] font-black uppercase text-neutral-400 text-right">Capital</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-beige-100">
                  {companies.filter(c => !c.is_admin).map((c, idx) => (
                    <tr key={c.id} className="hover:bg-beige-50/50 transition-colors">
                      ...
                      <td className="px-8 py-6">
                        <div className="font-black text-neutral-900">{c.company_name}</div>
                      </td>
                      <td className="px-8 py-6">
                        <div className="text-sm font-bold text-neutral-500">{c.first_name} {c.last_name}</div>
                      </td>
                      <td className="px-8 py-6 text-right">
                        <div className={`text-xl font-black tabular-nums ${c.balance < 0 ? 'text-red-500' : 'text-terracotta-700'}`}>
                    ...
                    <div key={i} className={`flex items-center justify-between p-5 rounded-2xl border-2 transition-all ${
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="min-w-0">
                          <div className="font-black text-neutral-900 truncate">{a.company_name}</div>
                          <div className="text-[10px] font-bold text-neutral-400 uppercase truncate">{a.first_name} {a.last_name}</div>
                        </div>
                      </div>
